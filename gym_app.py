...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    .stApp { background: linear-gradient(160deg, #f8f9ff 0%, #eef1ff 50%, #f0f7ff 100%); padding: 1.2rem; }
    .hero { background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%); border-radius:24px; padding:2.5rem 2rem; text-align:center; margin-bottom:1.5rem; position:relative; overflow:hidden; color: #fff; }
    ...
    .hero-title { font-size:2.2rem; font-weight:900; background:linear-gradient(90deg,#f7971e,#ffd200); -webkit-background-clip:text; -webkit-text-fill-color:transparent; margin:0; letter-spacing:-1px; }
    .hero-sub   { color:#cbd7ff; font-size:0.9rem; letter-spacing:3px; text-transform:uppercase; margin:0.4rem 0 1rem; }
    .credit-pill { display:inline-block; background:linear-gradient(90deg,#f7971e,#ffd200); color:#000; font-weight:800; font-size:0.72rem; padding:5px 18px; border-radius:20px; letter-spacing:1.5px; }
    ...
    .hero-stat-val   { font-size:1.1rem; font-weight:800; color:#ffd200; }
    .hero-stat-label { font-size:0.7rem; color:#d0d8ff; text-transform:uppercase; letter-spacing:1px; }
    ...
    .plan-badge { display:inline-block; background:linear-gradient(90deg,#f7971e,#ffd200); color:#000; font-size:0.7rem; font-weight:800; padding:3px 12px; border-radius:20px; letter-spacing:1px; margin-bottom:10px; }
    ...
    .stButton > button { width:100%; background:linear-gradient(90deg,#f7971e,#ffd200) !important; color:#000 !important; font-weight:800 !important; font-size:0.95rem !important; border:none !important; }
...
activity_map = {
    "Tidak aktif (sedikit/tdk olahraga)": 1.2,
...
activity_choice = st.selectbox("Pilih level aktivitas:", list(activity_map.keys()), index=act_idx)
...
if hitung:
    if not nama:
        ...
        bmr = calculate_bmr(berat, tinggi, usia, gender)
        st.session_state.kalori_bulat = round(bmr)
...
if st.session_state.get("sudah_hitung"):
    ...
    <div class="section-card">
        ...
        <div class="metrics-grid">
            ...
            <div class="metric-card" data-icon="🔥"><div class="metric-val">{bmr}</div><div class="metric-unit">kkal / hari</div><div class="metric-lbl">Basal Metabolic Rate (BMR)</div></div>
        </div>
    ...
    # KALORI / TDEE
    multipliers = [("Tidak aktif", 1.2), ("Olahraga ringan", 1.375), ("Olahraga sedang", 1.55), ("Olahraga berat", 1.725)]
    st.markdown(
        f"""
    <div class="section-card">
        ...
        <div style="margin-top:10px; color:#555">Berdasarkan pilihan aktivitas kamu saat ini, estimasi TDEE: <strong>{tdee} kkal/hari</strong></div>
    </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    if target == "🔥 Cutting — Turun berat badan":
        ...
        # jadwal & suplemen (sama seperti aslinya, ringkas di sini untuk keringkasan)
        jadwal = [
    ...
    artikel = [
        ...
        },
        {
            ...
            • Mood buruk dan sulit konsentrasi<br><br>
