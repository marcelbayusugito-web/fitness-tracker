...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .hero { background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%); border-radius:24px; padding:2.5rem 2rem; text-align:center; margin-bottom:1.5rem; position:relative; overflow:hidden; }
    ...
    .stButton > button { width:100%; background:linear-gradient(90deg,#f7971e,#ffd200) !important; color:#000 !important; font-weight:800 !important; font-size:0.95rem !important; border:none !important; border-radius:14px !important; padding:0.8rem !important; }
...
with col1:
    nama   = st.text_input("Nama Lengkap", placeholder="Contoh: Marcel Bayu Sugito", value=st.session_state.nama)
    usia   = st.number_input("Usia (tahun)", min_value=10, max_value=80, value=int(st.session_state.usia))
    gender = st.radio("Jenis Kelamin", ["Laki-laki", "Perempuan"], index=0 if st.session_state.gender == "Laki-laki" else 1, horizontal=True)
with col2:
    berat  = st.number_input("Berat Badan (kg)", min_value=30.0, max_value=200.0, value=float(st.session_state.berat), step=0.1)
    tinggi = st.number_input("Tinggi Badan (cm)", min_value=100.0, max_value=250.0, value=float(st.session_state.tinggi), step=0.1)

activity_map = {
    "Tidak aktif (sedikit/tidak olahraga)": 1.2,
    "Olahraga ringan (1-3 hari/minggu)": 1.375,
    "Olahraga sedang (3-5 hari/minggu)": 1.55,
    "Olahraga berat (6-7 hari/minggu)": 1.725,
}
act_idx = list(activity_map.values()).index(st.session_state.activity_factor) if st.session_state.activity_factor in activity_map.values() else 2
activity_choice = st.selectbox("Level Aktivitas:", list(activity_map.keys()), index=act_idx)
...
def calculate_bmr(weight, height_cm, age, gender):
    s = 5 if gender == "Laki-laki" else -161
...
if hitung:
    if not nama:
        ...
        st.session_state.activity_factor = activity_factor
        tinggi_meter = tinggi / 100
        st.session_state.bmi          = round(berat / (tinggi_meter ** 2), 1)
        st.session_state.kalori_bulat = round(calculate_bmr(berat, tinggi, usia, gender))
...
if st.session_state.get("sudah_hitung"):
    ...
    bmr             = st.session_state.kalori_bulat
    bmi_bulat       = st.session_state.bmi
    activity_factor = st.session_state.activity_factor
    ...
    <div class="section-card">
        ...
        <div class="metrics-grid">
            ...
            <div class="metric-card" data-icon="🔥"><div class="metric-val">{bmr}</div><div class="metric-unit">kkal / hari</div><div class="metric-lbl">Basal Metabolic Rate</div></div>
        </div>
    ...
    <div class="section-card">
        ...
        <div class="kalori-grid">
            <div class="kalori-item"><span class="kalori-name">😴 Tidak aktif</span><span class="kalori-val">{round(bmr * 1.2)} kkal</span></div>
            <div class="kalori-item"><span class="kalori-name">🚶 Olahraga ringan</span><span class="kalori-val">{round(bmr * 1.375)} kkal</span></div>
            <div class="kalori-item"><span class="kalori-name">🏃 Olahraga sedang</span><span class="kalori-val">{round(bmr * 1.55)} kkal</span></div>
            <div class="kalori-item"><span class="kalori-name">🏋️ Olahraga berat</span><span class="kalori-val">{round(bmr * 1.725)} kkal</span></div>
        </div>
        <div style="margin-top:10px; font-size:0.85rem; color:#555;">
            Berdasarkan level aktivitas kamu, estimasi TDEE: <strong style="color:#0f3460">{tdee} kkal/hari</strong>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if target == "🔥 Cutting — Turun berat badan":
        ...
        jadwal  = [
            ...
            {"hari": "Selasa", "tipe": "cardio",   "fokus": "🏃 Cardio HIIT", "exercises": [
            ...
            {"hari": "Rabu",   "tipe": "latihan",  "fokus": "🦵 Kaki & Bahu", "exercises": [
            ...
            {"hari": "Kamis",  "tipe": "cardio",   "fokus": "🚴 Cardio Steady State", "exercises": [
            ...
            {"hari": "Jumat",  "tipe": "latihan",  "fokus": "🏋️ Punggung & Bisep", "exercises": [
            ...
            {"hari": "Sabtu",  "tipe": "cardio",   "fokus": "🏃 Cardio + Core", "exercises": [
            ...
            {"hari": "Minggu", "tipe": "istirahat","fokus": "😴 Rest Day", "exercises": [
        ...
        kalori_target = round(tdee + 300)
        protein = round(berat * 1.8)
        label   = "💪 BULKING PLAN"
        catatan = "Surplus 300 kkal/hari = naik massa otot secara lean"
        jadwal  = [
            {"hari": "Senin",  "tipe": "latihan",  "fokus": "💪 Dada & Trisep (Heavy)", "exercises": [
                ...
                {"nama": "Close Grip Bench",     "detail": "4 set × 8 reps — istirahat 90 detik"},
                {"nama": "Skull Crusher",        "detail": "3 set × 10 reps — istirahat 60 detik"},
            ]},
            {"hari": "Selasa", "tipe": "latihan",  "fokus": "🦵 Kaki (Heavy)", "exercises": [
            ...
            {"hari": "Jumat",  "tipe": "latihan",  "fokus": "🎯 Bahu & Trap", "exercises": [
            ...
            {"hari": "Sabtu",  "tipe": "latihan",  "fokus": "🔁 Full Body & Core", "exercises": [
            ...
            {"hari": "Minggu", "tipe": "istirahat","fokus": "😴 Full Rest Day", "exercises": [
        ...
        jadwal  = [
            ...
            {"hari": "Selasa", "tipe": "cardio",   "fokus": "🏃 Cardio Ringan", "exercises": [
                {"nama": "Lari / Jalan Cepat","detail": "30 menit — pace sedang, zone 2"},
                {"nama": "Core Training",    "detail": "Plank 3×45 detik, Crunch 3×15, Leg Raise 3×12"},
            ]},
            {"hari": "Rabu",   "tipe": "latihan",  "fokus": "🦵 Lower Body", "exercises": [
            ...
            {"hari": "Kamis",  "tipe": "istirahat","fokus": "😴 Rest & Recovery", "exercises": [
                {"nama": "Stretching",   "detail": "15 menit atau yoga ringan"},
                {"nama": "Foam Rolling", "detail": "10 menit — fokus kaki dan punggung"},
            ]},
            {"hari": "Jumat",  "tipe": "latihan",  "fokus": "💪 Upper Body B", "exercises": [
                {"nama": "Pull Up / Lat Pulldown","detail": "4 set × 10 reps — istirahat 60 detik"},
            ...
            {"hari": "Sabtu",  "tipe": "cardio",   "fokus": "🏊 Cardio + Fleksibilitas", "exercises": [
    ...
    <div class="plan-card">
        ...
        <div class="plan-item">🔥 <span>Kalori target &nbsp;: <span class="plan-val">{kalori_target} kkal</span></span></div>
    ...
    for hari_data in jadwal:
        ...
        icon   = tipe_icon.get(tipe, "🏋️")
        badge  = tipe_badge.get(tipe, "badge-latihan")
        blabel = tipe_label.get(tipe, "Latihan")
        ...
        for ex in exs:
            exercise_html += (
                ...
                "<div>"
                f"<div class='exercise-name'>{ex['nama']}</div>"
                f"<div class='exercise-detail'>{ex['detail']}</div>"
                "</div></li>"
            )
        st.markdown(
            f"<div class='hari-card {tipe}'><div class='hari-title'>{icon} {hari} — {fokus} <span class='hari-badge {badge}'>{blabel}</span></div><ul class='exercise-list'>{exercise_html}</ul></div>",
            unsafe_allow_html=True,
        )
    ...
    for s in suplemen:
        st.markdown(f"<div class='suplemen-item'><div class='suplemen-name'>💊 {s['nama']}</div><div class='suplemen-detail'>{s['detail']}</div></div>", unsafe_allow_html=True)
    ...
    artikel = [
        {
            ...
            "isi": "<b>Berapa banyak protein yang kamu butuhkan?</b><br><br>Kebutuhan protein tergantung pada tujuan dan berat badan kamu:<br>• <b>Cutting:</b> 2.0 – 2.4 gram per kg berat badan<br>• <b>Bulking:</b> 1.6 – 2.0 gram per kg berat badan<br>• <b>Maintain:</b> 1.4 – 1.8 gram per kg berat badan<br><br><b>Sumber protein terbaik:</b><br>• Dada ayam (31g protein per 100g)<br>• Telur (13g protein per 100g)<br>• Ikan tuna (30g protein per 100g)<br>• Tempe (19g protein per 100g)<br>• Greek yogurt (10g protein per 100g)<br><br><b>Tips:</b> Sebarkan asupan protein ke 4-5 kali makan per hari untuk penyerapan optimal."
        },
        {
            "kategori": "🏋️ Latihan",
            ...
            "isi": "<b>Apa itu Progressive Overload?</b><br><br>Progressive overload artinya secara bertahap meningkatkan beban atau volume latihan dari waktu ke waktu, sehingga otot terus mendapat stimulus baru untuk berkembang.<br><br><b>Cara menerapkannya:</b><br>• Naikkan beban 2.5 – 5 kg setiap 1-2 minggu<br>• Tambah 1-2 reps tiap sesi jika beban belum bisa dinaikkan<br>• Tambah 1 set per latihan setiap bulan<br>• Kurangi waktu istirahat antar set<br><br><b>Contoh:</b><br>Minggu 1: Bench Press 50kg × 3 set × 10 reps<br>Minggu 2: Bench Press 50kg × 3 set × 12 reps<br>Minggu 3: Bench Press 52.5kg × 3 set × 10 reps<br><br><b>Ingat:</b> Catat setiap sesi latihan kamu agar bisa tracking progress dengan akurat."
        },
        {
            "kategori": "😴 Recovery",
            ...
            "isi": "<b>Apa yang terjadi saat kamu tidur?</b><br><br>Saat tidur, tubuh melepaskan Growth Hormone (GH) yang berfungsi memperbaiki serat otot yang rusak akibat latihan. Inilah mengapa tidur 7-9 jam sangat krusial.<br><br><b>Tanda-tanda overtrained:</b><br>• Performa latihan menurun<br>• Mudah lelah dan lesu<br>• Sering sakit (imunitas turun)<br>• Mood buruk dan sulit konsentrasi<br><br><b>Tips recovery terbaik:</b><br>• Tidur 7-9 jam per malam — prioritas utama<br>• Konsumsi protein sebelum tidur (kasein)<br>• Lakukan active recovery (jalan, stretching, foam rolling)<br>• Minimal 1-2 hari full rest per minggu<br><br><b>Ingat:</b> Rest day bukan hari malas — itu bagian dari program latihan kamu."
        },
        {
            "kategori": "🥤 Hidrasi",
            ...
            "isi": "<b>Berapa banyak air yang dibutuhkan?</b><br><br>• Minimal 2-3 liter per hari untuk orang aktif<br>• Tambah 500ml untuk setiap 30 menit latihan intens<br><br><b>Waktu minum yang optimal:</b><br>• Pagi hari: 1-2 gelas segera setelah bangun tidur<br>• Sebelum latihan: 500ml, 30 menit sebelumnya<br>• Selama latihan: 150-250ml setiap 15-20 menit<br>• Setelah latihan: 500ml untuk mengganti cairan yang hilang<br><br><b>Tanda dehidrasi saat latihan:</b><br>• Pusing dan kepala berat<br>• Kram otot tiba-tiba<br>• Detak jantung lebih cepat dari biasanya<br><br><b>Tips:</b> Bawa botol minum 1 liter ke gym dan targetkan habis 2x selama sesi latihan."
        },
        {
            "kategori": "🧠 Mental",
            ...
            "isi": "<b>Apa itu Mind-Muscle Connection?</b><br><br>Mind-muscle connection adalah kemampuan untuk secara sadar merasakan dan mengontrol otot yang sedang kamu latih.<br><br><b>Cara melatihnya:</b><br>• Turunkan beban 20-30% dan fokus pada sensasi otot<br>• Gerakkan secara perlahan — terutama fase negatif (turun/balik)<br>• Squeeze atau kontraksikan otot di titik puncak gerakan<br>• Hindari momentum — jangan ayun beban<br><br><b>Contoh — Bicep Curl:</b><br>❌ Salah: Angkat beban cepat menggunakan momentum bahu<br>✅ Benar: Angkat perlahan, rasakan bisep berkontraksi, squeeze di atas, turun 3 detik<br><br><b>Tips:</b> Letakkan tangan bebas di otot yang dilatih untuk membantu merasakan kontraksinya."
        },
    ]
    ...
    for art in artikel:
        with st.expander(f"{art['kategori']}  |  {art['judul']}"):
            ...
            <div style="font-size:0.88rem; color:#333; line-height:1.8;">{art['isi']}</div>
            """, unsafe_allow_html=True)
